...
final class PHPMailerTest extends TestCase{
	...
	public function testCustomValidator(){
		...
		$this->assertTrue(
			\PHPMailer\PHPMailer\validateAddress(
				'user@example.com',
				function($address){
		...
		$this->Mail->validator = function($address){
...
EOT;
	...
	public function testHtmlIso8859(){
		...
		$this->buildBody();
		$this->assertStringContainsString($check, $this->Mail->Body, 'ISO message body does not contain expected text');
		$this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
	}
...
EOT;
	...
	public function testShortBody(){
		...
		$this->Mail->preSend();
		$message = $this->Mail->getSentMIMEMessage();
		$this->assertFalse($this->Mail->hasLineLongerThanMax($message), 'Long line not corrected.');
		$this->assertStringNotContainsString(
			'Content-Transfer-Encoding: quoted-printable',
			$message,
	...
	public function testLineBreaks(){
		...
		$mixedsrc   = "hello\nWorld\rAgain\r\n";
		$target     = "hello\r\nWorld\r\nAgain\r\n";
		$this->assertEquals($target, $this->Mail->normalizeBreaks($unixsrc), 'UNIX break reformatting failed');
		$this->assertEquals($target, $this->Mail->normalizeBreaks($macsrc), 'Mac break reformatting failed');
		$this->assertEquals($target, $this->Mail->normalizeBreaks($windowssrc), 'Windows break reformatting failed');
		$this->assertEquals($target, $this->Mail->normalizeBreaks($mixedsrc), 'Mixed break reformatting failed');
	...
	public function testLineLength(){
		...
		$this->assertTrue(
			$this->Mail->hasLineLongerThanMax($oklen.$badlen.$oklen),
	...
	public function testMiscellaneous(){
		...
		$this->assertEquals($this->Mail->normalizeBreaks($b1), $t1, 'Failed to normalize line breaks (1)');
		$this->assertEquals($this->Mail->normalizeBreaks($b2), $t1, 'Failed to normalize line breaks (2)');
	...
	public function testPopBeforeSmtpGood(){
		...
		$this->assertTrue(
			(new POP3)->authorise('localhost', 1100, 10, 'user', 'test', $this->Mail->loglevel),
	...
	public function testPopBeforeSmtpBad(){
		...
		$this->assertFalse(
			(new POP3)->authorise('localhost', 1101, 10, 'user', 'xxx', $this->Mail->loglevel),
